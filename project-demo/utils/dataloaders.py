...
class LoadImages:
    ...
    # path为data\\images\\bus.jpg[640*640]
    def __init__(self, path, img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        files = []

        # isinstance()函数，是python中的一个内置函数，用来判断一个函数是否是一个已知的类型
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:  # 相对路径得到绝对路径
            ...
            if '*' in p:
                ...
                files.append(p)  # files [C:\\Users\\]
            else:
                raise FileNotFoundError(f'{p} does not exist')

        images = [x for x in files if x.split('.')[-1].lower() in IMG_FORMATS]  # 遍历所有元素, 分割图片文件名
        videos = [x for x in files if x.split('.')[-1].lower() in VID_FORMATS]  # 遍历所有元素, 分割视频文件名
